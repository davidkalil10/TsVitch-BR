...
void XtreamAPI::getVodCategories(CategoriesCallback callback) {
    ...
    makeRequest(url, [callback](const json& response, bool success, const std::string& error) {
        ...
        try {
            ...
            callback(std::vector<XtreamCategory>{}, false, "Error parsing VOD categories: " + std::string(e.what()));
        }
    });
}
...
void XtreamAPI::getSeriesCategories(CategoriesCallback callback) {
    ...
    makeRequest(url, [callback](const json& response, bool success, const std::string& error) {
        ...
        try {
            ...
            callback(std::vector<XtreamCategory>{}, false, "Error parsing Series categories: " + std::string(e.what()));
        }
    });
}

void XtreamAPI::getVodStreams(const std::string& categoryId, VodCallback callback) {
    if (!isConfigured()) {
        callback(std::vector<XtreamVod>{}, false, "Xtream credentials not configured");
        return;
    }
    ...
    makeRequest(url, [callback](const json& response, bool success, const std::string& error) {
        if (!success) {
            callback(std::vector<XtreamVod>{}, false, error);
            return;
        }
        
        try {
            std::vector<XtreamVod> vods;
            if (response.is_array()) {
                for (const auto& item : response) {
                    XtreamVod vod;
                    vod.num = item.value("num", "");
                    vod.name = item.value("name", "");
                    vod.stream_type = item.value("stream_type", "");
                    vod.stream_id = item.value("stream_id", "");
                    vod.stream_icon = item.value("stream_icon", "");
                    vod.rating = item.value("rating", "");
                    vod.rating_5based = item.value("rating_5based", "");
                    vod.added = item.value("added", "");
                    vod.category_id = item.value("category_id", "");
                    vod.container_extension = item.value("container_extension", "");
                    vod.custom_sid = item.value("custom_sid", "");
                    vod.direct_source = item.value("direct_source", "");
                    vods.push_back(vod);
                }
            }
            brls::Logger::info("XtreamAPI: Retrieved {} VOD streams", vods.size());
            callback(vods, true, "");
        } catch (const std::exception& e) {
            callback(std::vector<XtreamVod>{}, false, "Error parsing VOD streams: " + std::string(e.what()));
        }
    });
}
...
void XtreamAPI::getSeries(const std::string& categoryId, SeriesCallback callback) {
    ...
    brls::Logger::info("XtreamAPI: Getting Series from: {}", url);
    ...
    makeRequest(url, [callback](const json& response, bool success, const std::string& error) {
        ...
        try {
            ...
            if (response.is_array()) {
                for (const auto& item : response) {
                    XtreamSeries series;
                    series.num = item.value("num", "");
                    series.name = item.value("name", "");
                    series.series_id = item.value("series_id", "");
                    series.cover = item.value("cover", "");
                    series.plot = item.value("plot", "");
                    series.cast = item.value("cast", "");
                    series.director = item.value("director", "");
                    series.genre = item.value("genre", "");
                    series.releaseDate = item.value("releaseDate", "");
                    series.last_modified = item.value("last_modified", "");
                    series.rating = item.value("rating", "");
                    series.rating_5based = item.value("rating_5based", "");
                    series.backdrop_path = item.value("backdrop_path", "");
                    series.youtube_trailer = item.value("youtube_trailer", "");
                    series.episode_run_time = item.value("episode_run_time", "");
                    series.category_id = item.value("category_id", "");
                    seriesList.push_back(series);
                }
            }
            brls::Logger::info("XtreamAPI: Retrieved {} Series", seriesList.size());
            callback(seriesList, true, "");
        } catch (const std::exception& e) {
...
void XtreamAPI::getAllVodStreams(VodCallback callback) {
    getVodStreams("", callback);
}

void XtreamAPI::getAllSeries(SeriesCallback callback) {
    getSeries("", callback);
}
