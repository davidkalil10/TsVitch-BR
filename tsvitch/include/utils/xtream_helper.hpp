...
public:
    ...
    // Callbacks for new types
    using VodCallback = std::function<void(const std::vector<XtreamVod>&, bool success, const std::string& error)>;
    using SeriesCallback = std::function<void(const std::vector<XtreamSeries>&, bool success, const std::string& error)>;

    // Ottieni le categorie VOD e Series
    void getVodCategories(CategoriesCallback callback);
    void getSeriesCategories(CategoriesCallback callback);

    // Ottieni i contenuti VOD e Series
    void getVodStreams(const std::string& categoryId, VodCallback callback);
    void getSeries(const std::string& categoryId, SeriesCallback callback);
    void getAllVodStreams(VodCallback callback);
